...
const drawMetaCard = (page: PDFPage, x: number, yTop: number, width: number, height: number, label: string, value: string, bodyFont: PDFFont, boldFont: PDFFont) => {
  ...
  let lineY = yTop - 52;
  lines.forEach(line => {
    page.drawText(line, { x: x + 14, y: lineY, size: 11, font: boldFont, color: COLORS.text });
    lineY -= 15;
  });
};
...
const drawKeyValueRows = (page: PDFPage, x: number, yTop: number, width: number, rows: Array<{ key: string; note: string; value: string; emphasize?: boolean }>, bodyFont: PDFFont, boldFont: PDFFont) => {
  ...
  rows.forEach((row, idx) => {
    ...
    page.drawText(row.key, { x: x + 14, y: rowTop - 24, size: 8.6, font: boldFont, color: COLORS.text });
    page.drawText(row.note, { x: x + 14, y: rowTop - 40, size: 7.2, font: row.emphasize ? boldFont : bodyFont, color: COLORS.textSoft });
    const valueWidth = boldFont.widthOfTextAtSize(row.value, 9.4);
    page.drawText(row.value, { x: x + width - 14 - valueWidth, y: rowTop - 24, size: 9.4, font: boldFont, color: COLORS.text });
  });
};
...
const drawTable = (
  ...
  columns.forEach((col, idx) => {
    const textWidth = boldFont.widthOfTextAtSize(col.label, headerSize);
    const textX = col.align === 'right' ? colX + col.width - 12 - textWidth : colX + 12;
    page.drawText(col.label, { x: textX, y: yTop - 18, size: headerSize, font: boldFont, color: COLORS.textSoft, characterSpacing: idx === 0 ? 0.8 : 1.2 });
    colX += col.width;
  });
  ...
  rows.forEach((row, rowIndex) => {
    ...
    row.forEach((cell, idx) => {
      ...
      if (align === 'right') {
        const cellWidth = bodyFont.widthOfTextAtSize(cell, bodySize);
        page.drawText(cell, { x: cellX + columns[idx].width - 12 - cellWidth, y: top - 22, size: bodySize, font, color: COLORS.text });
      } else {
        page.drawText(cell, { x: cellX + 12, y: top - 22, size: bodySize, font, color: COLORS.text });
      }
...
export async function generateTaxSummaryPdfBytes(data: TaxSummaryPdfData): Promise<Uint8Array> {
  ...
  // These custom OTFs throw a fontkit RangeError when subsetting on export.
  // Embed the full font files instead so Tax Summary PDF export is stable across devices.
  const bodyFont = await pdfDoc.embedFont(reportRegularOtf, { subset: false });
  const boldFont = await pdfDoc.embedFont(reportBoldOtf, { subset: false });
  const kickerFont = await pdfDoc.embedFont(appRegularOtf, { subset: false });
  ...
  // Page 1
  page1.drawText('MONIEZI PRO FINANCE', { x: margin, y: pageHeight - margin + 4, size: 11, font: kickerFont, color: COLORS.blue, characterSpacing: 2.1 });
  page1.drawText('MONIEZI', { x: margin, y: pageHeight - 92, size: 40, font: boldFont, color: COLORS.text });
  page1.drawText(`Tax Prep Package Summary ${data.taxYear}`, { x: margin, y: pageHeight - 126, size: 22, font: boldFont, color: COLORS.text });
  ...
  // Page 2
  page2.drawText('MONIEZI TAX PREP PACKAGE', { x: margin, y: pageHeight - margin + 4, size: 11, font: kickerFont, color: COLORS.blue, characterSpacing: 2.1 });
  page2.drawText('Documentation Status & Expense Breakdown', { x: margin, y: pageHeight - 92, size: 25, font: boldFont, color: COLORS.text });
  ...
  // Page 3
  page3.drawText('MONIEZI TAX PREP PACKAGE', { x: margin, y: pageHeight - margin + 4, size: 11, font: kickerFont, color: COLORS.blue, characterSpacing: 2.1 });
  page3.drawText('Mileage & Filing Checks', { x: margin, y: pageHeight - 92, size: 25, font: boldFont, color: COLORS.text });
  ...
  if (data.hasMileageRows) {
    ...
    page3.drawText('No mileage trips were recorded for this tax year.', { x: margin + 18, y: section4Top - 132, size: 10.5, font: bodyFont, color: COLORS.textSoft });
  }
  ...
  data.attentionItems.slice(0, 4).forEach(item => {
    ...
    wrapped.forEach((line, idx) => {
      ...
      page3.drawText(line, { x: margin + splitWidth + splitGap + 28, y: noteY, size: 9.2, font: bodyFont, color: COLORS.text });
      noteY -= 13;
    });
    noteY -= 12;
  });
  ...
  pdfDoc.setTitle(`MONIEZI Tax Prep Package Summary ${data.taxYear}`);
  pdfDoc.setAuthor('MONIEZI');
  pdfDoc.setCreator('MONIEZI Pro Finance');
  pdfDoc.setProducer('MONIEZI Pro Finance');
  pdfDoc.setSubject(`Tax Prep Package Summary ${data.taxYear}`);
  return pdfDoc.save();
}
